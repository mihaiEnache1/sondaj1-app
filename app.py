...
def download_json_from_gcs(bucket_name, survey_folder):
    bucket = storage_client.bucket(bucket_name)
    json_filename = f"{survey_folder}\expressions_processed.json"  # Ajustare separator pentru compatibilitate
    blob = bucket.blob(json_filename)
    data = blob.download_as_text()
    return json.loads(data)

# Obținerea parametrilor din URL
query_params = st.query_params
selected_survey = query_params["survey"]

if selected_survey:
    try:
        data = download_json_from_gcs(bucket_name, selected_survey)
        ...
        if st.button('Trimite răspunsurile'):
            ...
            responses_filename = f"responses_{selected_survey}_{timestamp}.json"
        ...
        st.error(f"A apărut o eroare la încărcarea sondajului: {e}")
else:
    st.write("Nu a fost specificat niciun sondaj. Te rugăm să accesezi link-ul corect.")
